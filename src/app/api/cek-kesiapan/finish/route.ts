...
import { finishCekKesiapan } from "@/lib/cek-kesiapan";
...
export async function POST(req: NextRequest) {
  ...
  const targetKode = String(body.targetKode ?? "").trim();
  const answers = (body.answers as AnswerInput[]) ?? [];
  if (!targetKode || !Array.isArray(answers)) {
    return NextResponse.json({ error: "targetKode + answers[] wajib" }, { status: 400 });
  }
  ...
  // Update mastery per blind-spot kode (kalau jawab benar = siap, salah = remediasi)
  ...
  for (const [kode, agg] of byKode.entries()) {
    ...
    masteryUpdates.push({
      ...
      confidence: 0.6, // confidence sedang — hasil warmup, bukan full diagnostic
  ...
  const decision = finishCekKesiapan(targetKode, responses);
